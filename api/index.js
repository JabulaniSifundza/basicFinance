...
app.post('/api/profile', async(req, res)=>{
  ...
  try{
	...
	res.status(200).json(data)
  	...
  	res.status(500).json({"error": error.name, "message": error.message, "ticker-attempted": ticker})
  }
})
        ...
        switch (btn_msg) {
          case 'start_get_help':
            message = {
              messaging_product: "whatsapp",
              to: from,
              text: { body: "You selected 'Get Help'. How can we assist you further?" },
            };
            break;
          case 'start_data_bundles':
            message = {
              messaging_product: "whatsapp",
              to: from,
              text: { body: "You selected 'Data Bundles'. Here are our available data bundles..." },
            };
            break;
          ...
          default:
            message = {
              messaging_product: "whatsapp",
              to: from,
              text: { body: "I'm sorry, I didn't understand that. Could you please rephrase or select one of the options?" },
            };
            break;
        }
      } else {
